...
func run(cfg appConfig) (err error) {
	...
	// Satisfies CleanStaleScratch's caller contract: the cross-process
	// lock (see lock.go, held by the caller for the entirety of run())
	// guarantees only one run() is ever active, so nothing left in
	// grid-scratch can still be in use. Always safe here, even before
	// deciding whether a real pass is due.
	demgrid.CleanStaleScratch(cfg.demCacheDir)
